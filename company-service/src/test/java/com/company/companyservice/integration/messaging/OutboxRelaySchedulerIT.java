...
import com.company.companyservice.infrastructure.messaging.KafkaOutboxSender;
import com.company.companyservice.infrastructure.messaging.OutboxRelayScheduler;
import com.company.companyservice.infrastructure.messaging.OutboxRelayService;
import com.company.companyservice.infrastructure.persistence.command.JpaOutboxRelayRepository;
...
class OutboxRelaySchedulerIT {
    ...
    void relay_sendsPendingRowToKafkaAndMarksPublished() throws Exception {
        ...
        String payload = "{\"eventId\":\"" + UUID.randomUUID() + "\",\"eventType\":\"CompanyCreatedEvent\","
                + "\"aggregateId\":\"" + aggregateId + "\",\"aggregateType\":\"Company\","
                + "\"timestamp\":\"2026-01-01T00:00:00Z\",\"version\":1,\"payload\":{}}";

        OutboxEventEntity pending = new OutboxEventEntity(
                UUID.randomUUID(), aggregateId, "CompanyCreatedEvent",
                "company-events", payload, "PENDING", Instant.now(), null);
    ...
    void relay_skipsAlreadyPublishedRows() {
        ...
        OutboxEventEntity published = new OutboxEventEntity(
                UUID.randomUUID(), aggregateId, "CompanyCreatedEvent",
                "company-events", "{}", "PUBLISHED", Instant.now(), Instant.now());
