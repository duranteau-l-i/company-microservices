...
import com.company.userservice.infrastructure.messaging.KafkaOutboxSender;
import com.company.userservice.infrastructure.messaging.OutboxRelayScheduler;
import com.company.userservice.infrastructure.messaging.OutboxRelayService;
import com.company.userservice.infrastructure.persistence.command.JpaOutboxRelayRepository;
...
class OutboxRelaySchedulerIT {
    ...
    void relay_sendsPendingRowToKafkaAndMarksPublished() throws Exception {
        ...
        String payload = "{\"eventId\":\"" + UUID.randomUUID() + "\",\"eventType\":\"UserCreatedEvent\","
                + "\"aggregateId\":\"" + aggregateId + "\",\"aggregateType\":\"User\","
                + "\"timestamp\":\"2026-01-01T00:00:00Z\",\"version\":1,\"payload\":{}}";

        OutboxEventEntity pending = new OutboxEventEntity(
                UUID.randomUUID(), aggregateId, "UserCreatedEvent",
                "user-events", payload, "PENDING", Instant.now(), null);
        outboxRepository.save(pending);

        try (KafkaConsumer<String, String> consumer = newConsumer("user-events")) {
